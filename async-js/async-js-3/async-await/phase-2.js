...
async function workout(totalTime){
    try{
      const p1 = await stretch(totalTime)
      const p2 = await runOnTreadmill(p1)
      const p3 = await liftWeights(p2)
      console.log(`done working out with ${p3/1000} seconds left`)
    } catch(err){
